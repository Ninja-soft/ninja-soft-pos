...
- **Medios de pago — Mobbex (cobro por QR).** Nueva integración: conectás Mobbex pegando API Key + Access Token en Configuración → Medios de pago; en el POS aparece "Cobrar con QR · Mobbex" (mismo flujo que MP: elegir tarjeta/plan por botón con total calculado, o recargo único). Edge Functions `mobbex_create_qr` (crea el checkout y devuelve la URL para QR) y `mobbex_webhook` (actualiza el estado). Reusa `payment_secrets`, `mp_payment_intents` (provider 'mobbex') y los planes por medio (H27). El botón solo aparece con el medio conectado. *Pendiente: deploy de las Edge Functions vía MCP + credenciales reales para probar.*
