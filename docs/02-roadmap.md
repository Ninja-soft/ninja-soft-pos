...
- [~] **H31 — Cuenta corriente y grupos de clientes.** — *Criterio principal cumplido (2026-06-07). Restan: deuda en historial desde el selector del POS y grupos para precios/promos/riesgo.*
  ...
  - [x] **Escaneo de DNI argentino para alta rápida de cliente** *(agregado 2026-06-06; hecho 2026-06-08, PR #__)*: lee el código **PDF417** del frente del DNI (scanner 2D USB tipo teclado vía `useScanner`, cámara con `BarcodeDetector` cuando soporta `pdf417`, o pegado manual) y parsea **offline** los campos separados por `@` (apellido, nombre, sexo, N° de DNI, fecha de nacimiento) con `parseDni` (`lib/customers/dniParse.ts`, +16 tests) para autocompletar la ficha del cliente en alta/edición (`CustomerFormModal`) y el alta rápida del selector del POS (`createQuick`). Sin servicios ni registros gubernamentales — el dato vive en el propio código del documento. Componente `DniScanModal` con **validación visual** de los datos parseados antes de guardar (no autocompleta sin confirmación); degrada honestamente si el navegador no lee `pdf417` por cámara.
