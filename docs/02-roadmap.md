...
- [ ] **H47 — Menú gastronómico, modificadores y cursos.**
  ...
  - [~] Menú por horario/canal: desayuno, almuerzo, merienda, cena, happy hour, delivery. — *Base hecha (local): **menús por franja** con horario y productos. Migración `20260609150000_gastro_menus` (aplicada en remoto): `menus` + `menu_windows` (día 0-6 + rango en minutos, hora local AR) + `product_menus` (asignación) con RLS tenant (escritura sólo owner/manager) + resolver **`active_menu_ids()`** (SECURITY INVOKER, vía RLS) que marca los menús vigentes según día+hora; menú sin ventanas = siempre disponible. UI: sección **Configuración → "Menús por horario"** (`MenusManager`, gateada por `dining_enabled`): CRUD de menús, franjas (día + desde/hasta), asignación de productos (buscador + checklist) y badge **"Activo ahora"** en vivo. Helper de tiempo puro y testeado (`lib/gastro/menuTime.ts` + `tests/unit/menu-time.test.ts`). **Follow-up:** que el POS **filtre la carta** al menú vigente (consume `active_menu_ids` + `product_menus`); menú por CANAL (mostrador/delivery) y ventanas que cruzan medianoche (happy hour 22→02).*
