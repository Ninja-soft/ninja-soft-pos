...
- [~] **H53 — Núcleo del motor de reglas.** — *Motor declarativo (puro + testeado) + definición + admin + **aplicación en el POS** (el descuento se aplica al carrito en vivo y se persiste en `create_sale`). Falta: alcance por categoría en el POS (hoy carrito/producto), combinables (H54) y segmentos (H55).*
  ...
  - [~] Evaluación en el carrito en vivo, con prioridad, **combinables o exclusivas**, y tope de descuento. — *Hecho (local, en PR para revisar): el POS evalúa las promos activas contra el carrito en vivo (motor + contexto día/hora AR), muestra la mejor promo (**"<nombre> −$X"** en verde) y la descuenta del total. Al cobrar, viaja a `create_sale` en un **canal separado** (`p_promo_discount`/`p_promo_id`/`p_promo_name`) que **NO** pasa por el gating de la feature 'descuentos' ni por el tope de descuento por rol, se acota server-side y se **persiste** en la venta (`sales.promo_id`/`promo_name`/`promo_discount`). Migración `20260609250000_create_sale_promo` (recreada desde la def viva, backward-compatible). **Exclusiva** (la de mayor descuento; combinables = H54). **v1 del POS: alcance carrito/producto** (categoría por línea = follow-up). El server **confía** en el monto del cliente (como con precios/modificadores), sólo lo acota.*
  ...
  - [~] *Criterio:* "Miércoles 30% en bebidas de 18 a 21hs" se configura sin código y aplica solo en ese rango. — *Se configura sin código y el motor + POS lo aplican en la venta dentro del rango. (El alcance por **categoría** en el POS — "bebidas" — es el follow-up de v1; con alcance carrito/producto ya aplica.)*
