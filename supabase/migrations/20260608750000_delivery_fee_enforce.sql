-- =============================================================================
-- 20260608750000_delivery_fee_enforce  (FIX 🟠 — costo de envío inviolable)
--
-- El delivery_fee se snapshotea autoritativo en delivery_orders al crear el
-- pedido, pero al COBRAR viajaba como línea de carrito client-side -> un cajero
-- o un cliente alterado podía quitarla y cobrar envío $0 (y el audit mentía).
--
-- create_sale ahora, en la rama de delivery (p_delivery_order_id), lee
-- delivery_orders.delivery_fee del pedido bloqueado FOR UPDATE y lo agrega de
-- forma AUTORITATIVA: lo suma a v_subtotal/v_total (después del redondeo/descuento
-- — el envío no se descuenta ni se redondea) y lo inserta como línea de venta
-- "Costo de envío" (espeja la prima de garantía). El POS deja de inyectar la
-- línea; el fee sale del pedido, no del carrito -> si no se paga, insufficient_payment.
--
-- Re-aplica el cuerpo COMPLETO de create_sale preservando todo lo anterior:
-- assert_account_active, validación QR (status/monto/idempotencia/anchor), rama
-- atómica de mesa (p_table_order_id), extras (tip/professional/warranty/pack/
-- modifiers), redondeo, stock (kits/variantes/seriales), gating de features.
-- =============================================================================

CREATE OR REPLACE FUNCTION public.create_sale(p_items jsonb, p_payments jsonb, p_discount_total numeric DEFAULT 0, p_customer_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text, p_extras jsonb DEFAULT '[]'::jsonb, p_table_order_id uuid DEFAULT NULL::uuid, p_delivery_order_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if p_delivery_order_id is not null then
    ...
    -- Costo de envío AUTORITATIVO: lo fija el server desde el pedido (clamp >= 0),
    -- no el carrito. Se agrega a v_total y como línea de venta más abajo.
  ...
  -- Costo de envío server-side (Bug 🟠): se suma DESPUÉS del redondeo/descuento
  -- (el envío no se descuenta ni se redondea) y ANTES de validar pagos. Se refleja
  -- en v_subtotal y v_total simétricamente para que sales.discount_total
  -- (v_subtotal - v_total) quede intacto. Así v_paid debe cubrir items + fee real.
  ...
  -- Línea AUTORITATIVA de costo de envío (Bug 🟠): la agrega el SERVER desde
  -- delivery_orders.delivery_fee (no el carrito), espejando cómo la prima de
  -- garantía entra como línea. qty 1, sin product_id, precio = fee. Ya está
  -- contabilizada en v_subtotal/v_total arriba.
  if v_delivery_fee > 0 then
    insert into sale_items (sale_id, product_id, variant_id, product_name, sku, serial,
                            quantity, unit_price, discount, subtotal, modifiers)
