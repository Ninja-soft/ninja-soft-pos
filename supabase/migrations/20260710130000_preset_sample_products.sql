-- =============================================================================
-- 20260710130000_preset_sample_products
--
-- Pedidos del dueño sobre "Configuración rápida por rubro":
--   1) El wizard debe PREGUNTAR si precargar los productos de muestra
--      → apply_industry_preset gana p_seed_catalog boolean default true.
--        En false NO siembra categorías ni ítems (sí aplica defaults del POS y
--        la suite gastronómica del preset, que no son "productos de prueba").
--   2) Poder BORRAR los productos de prueba después
--      → products.from_preset (text, key del preset que lo sembró) +
--        preset_sample_count() / delete_preset_samples() (baja lógica, auditada)
--        + backfill por nombre exacto de los ítems históricos de c_data.
--
-- Además: el CHECK de tenants.industry seguía en 6 rubros (23514 al elegir
-- heladería/cafetería/…): se recrea con los 12 verticales, alineado con
-- _assert_valid_industry (20260710120000) y lib/verticals/config.ts.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- A) tenants.industry: CHECK con los 12 verticales.
-- -----------------------------------------------------------------------------
alter table tenants drop constraint if exists tenants_industry_check;
alter table tenants add constraint tenants_industry_check check (
  industry in (
    'kiosco','textil','retail','restaurante','heladeria','cafeteria',
    'panaderia','peluqueria','estetica','servicios','pyme','otro'
  )
);

-- -----------------------------------------------------------------------------
-- B) products.from_preset: key del preset que sembró el producto (null = real).
-- -----------------------------------------------------------------------------
alter table products add column if not exists from_preset text;

-- -----------------------------------------------------------------------------
-- C) apply_industry_preset(p_preset, p_sells, p_seed_catalog default true).
--    Se recrea completa (misma lógica que 20260710120000) con:
--      * p_seed_catalog=false → saltea categorías + ítems.
--      * los ítems sembrados quedan marcados con from_preset = p_preset.
--    Se elimina la firma vieja de 2 args para que PostgREST no ambigüe (las
--    llamadas con 2 args siguen andando por el default del tercero).
-- -----------------------------------------------------------------------------
drop function if exists apply_industry_preset(text, text);

-- B) apply_industry_preset con presets de comercio general.
-- -----------------------------------------------------------------------------
create or replace function apply_industry_preset(
  p_preset text, p_sells text, p_seed_catalog boolean default true
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  [
    {"preset":"kiosco","cat":"Golosinas","item":"Alfajor","price":1200,"kind":"product"},
    {"preset":"kiosco","cat":"Golosinas","item":"Chocolate","price":2500,"kind":"product"},
    {"preset":"kiosco","cat":"Golosinas","item":"Chicles","price":800,"kind":"product"},
    {"preset":"kiosco","cat":"Bebidas","item":"Gaseosa lata","price":1600,"kind":"product"},
    {"preset":"kiosco","cat":"Bebidas","item":"Agua 500ml","price":1200,"kind":"product"},
    {"preset":"kiosco","cat":"Bebidas","item":"Jugo","price":1300,"kind":"product"},
    {"preset":"kiosco","cat":"Cigarrillos","item":"Cigarrillos box","price":4500,"kind":"product"},
    {"preset":"kiosco","cat":"Snacks","item":"Papas fritas","price":2200,"kind":"product"},
    {"preset":"kiosco","cat":"Snacks","item":"Maní","price":1500,"kind":"product"},

    {"preset":"minimarket","cat":"Almacén","item":"Arroz 1 kg","price":2200,"kind":"product"},
    {"preset":"minimarket","cat":"Almacén","item":"Fideos 500 g","price":1800,"kind":"product"},
    {"preset":"minimarket","cat":"Almacén","item":"Aceite 1L","price":4500,"kind":"product"},
    {"preset":"minimarket","cat":"Almacén","item":"Azúcar 1 kg","price":1900,"kind":"product"},
    {"preset":"minimarket","cat":"Bebidas","item":"Gaseosa 1.5L","price":2600,"kind":"product"},
    {"preset":"minimarket","cat":"Bebidas","item":"Agua 2L","price":1800,"kind":"product"},
    {"preset":"minimarket","cat":"Lácteos","item":"Leche 1L","price":1900,"kind":"product"},
    {"preset":"minimarket","cat":"Lácteos","item":"Yogur","price":1400,"kind":"product"},
    {"preset":"minimarket","cat":"Limpieza","item":"Lavandina 1L","price":1600,"kind":"product"},
    {"preset":"minimarket","cat":"Limpieza","item":"Detergente","price":2200,"kind":"product"},

    {"preset":"retail","cat":"Indumentaria","item":"Remera","price":12000,"kind":"product"},
    {"preset":"retail","cat":"Indumentaria","item":"Pantalón","price":25000,"kind":"product"},
    {"preset":"retail","cat":"Indumentaria","item":"Buzo","price":28000,"kind":"product"},
    {"preset":"retail","cat":"Calzado","item":"Zapatillas","price":60000,"kind":"product"},
    {"preset":"retail","cat":"Accesorios","item":"Gorra","price":9000,"kind":"product"},
    {"preset":"retail","cat":"Accesorios","item":"Cinturón","price":8000,"kind":"product"},
...
begin
  ...
  -- p_seed_catalog=false → el dueño pidió NO precargar productos de muestra:
  -- salteamos categorías e ítems (settings/gastronomía corren igual).
  ...
  loop
    ...
    values (
      ...
      v_item.station,
      -- Marca de "producto de muestra": habilita borrarlos después.
      p_preset
    );
    v_items_made := v_items_made + 1;
    v_fav_ord := v_fav_ord + 1;
  end loop;
  end if;  -- p_seed_catalog
...
revoke execute on function apply_industry_preset(text, text, boolean) from public, anon;
grant  execute on function apply_industry_preset(text, text, boolean) to authenticated;


-- -----------------------------------------------------------------------------
-- D) Backfill: marca como muestra los productos históricos sembrados por
--    presets ANTES de existir from_preset. Match conservador: nombre EXACTO
--    (case-insensitive) de un ítem del catálogo de presets, activo y sin marca.
--    (Si el dueño creó a mano un producto con idéntico nombre de muestra, quedará
--    marcado; el borrado es baja lógica y requiere confirmación explícita.)
-- -----------------------------------------------------------------------------
update products p
   set from_preset = 'backfill'
 where p.deleted_at is null
   and p.from_preset is null
   and lower(p.name) in (
     'alfajor','chocolate','chicles','gaseosa lata','agua 500ml','jugo',
     'cigarrillos box','papas fritas','maní',
     'arroz 1 kg','fideos 500 g','aceite 1l','azúcar 1 kg','gaseosa 1.5l',
     'agua 2l','leche 1l','yogur','lavandina 1l','detergente',
     'remera','pantalón','buzo','zapatillas','gorra','cinturón',
     'helado 1/4 kg','helado 1/2 kg','helado 1 kg','cucurucho','vasito',
     'postre helado','café chico','café mediano','café grande','cortado',
     'medialuna','tostado','pan (kg)','criollos (kg)','factura surtida',
     'bizcocho','gaseosa 1.5l','corte','corte + barba','color','reflejos',
     'peinado','brushing','esmaltado semipermanente','kapping',
     'limpieza facial','media pierna','cejas','lavado exterior',
     'lavado completo','encerado','limpieza de tapizados','aspirado',
     'service básico','cambio de aceite','diagnóstico','mano de obra (hora)',
     'aceite (litro)','filtro de aceite','hora de consultoría',
     'visita técnica','presupuesto','servicio mensual',
     'empanada','provoleta','milanesa con papas','bife de chorizo','ñoquis',
     'flan','helado','picada','rabas','hamburguesa completa','fernet con cola',
     'gin tonic','cerveza pinta','cerveza porrón','pollo al spiedo',
     'medio pollo','milanesa','docena de empanadas','ensalada mixta',
     'hamburguesa doble','pizza muzzarella','pizza especial','nuggets'
   );

-- -----------------------------------------------------------------------------
-- E) preset_sample_count(): ¿cuántos productos de muestra activos tiene el
--    tenant? (para mostrar/ocultar el botón "Borrar productos de prueba").
-- -----------------------------------------------------------------------------
create or replace function preset_sample_count()
returns int
language sql
stable
security definer
...
as $$
  ...
  where tenant_id = current_tenant_id()
    and from_preset is not null
    and deleted_at is null;
$$;

revoke all on function preset_sample_count() from public, anon;
grant execute on function preset_sample_count() to authenticated;

-- -----------------------------------------------------------------------------
-- F) delete_preset_samples(): baja lógica de TODOS los productos de muestra del
--    tenant activo. Guard owner/manager + auditoría. Devuelve cuántos dio de baja.
-- -----------------------------------------------------------------------------
create or replace function delete_preset_samples()
returns int
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  insert into audit_logs (tenant_id, actor_user_id, entity_type, entity_id, action, after_data)
  values (v_tenant, auth.uid(), 'products', v_tenant, 'preset_samples_deleted',
...
revoke all on function delete_preset_samples() from public, anon;
grant execute on function delete_preset_samples() to authenticated;
